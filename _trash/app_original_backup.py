...
def extract_text_from_image_json(image_bgr: np.ndarray, image_id: str = None) -> dict:
    ...
    NEW APPROACH: First detect license plates as objects, then extract text from cropped plates.
            ...
            if plate_text and plate_text.strip():
                ...
                if _is_valid_indian_license_plate(cleaned_plate):
                    ...
                    if _validate_license_plate_in_image(plate_crop, cleaned_plate):
                        ...
                        car_color = _detect_car_color_around_plate(image_bgr, x1, y1, x2, y2)
                        
                        result["text_extraction"]["all_objects"].append({
                            "object_id": f"license_plate_{i}",
                            ...
                            "color": car_color,  # Use car color, not plate color
                            "text_found": [{
                                "text": cleaned_plate,
                                "type": "license_plate",
                                "confidence": 0.9,
        ...
        if full_image_text:
            ...
            for text_item in full_image_text:
                if "plate" in text_item["method"]:
                    ...
                    if _is_valid_indian_license_plate(plate_text):
                        ...
                        if plate_text not in existing_plates:
                            ...
                            result["text_extraction"]["summary"]["license_plates_found"] += 1
                            print(f"[DEBUG] Found additional license plate in full image text: {plate_text}")
                else:
...
def _extract_text_from_license_plate_crop(plate_crop: np.ndarray) -> str:
    """
    Extract text from a cropped license plate region using enhanced OCR techniques.
    Enhanced for accurate license plate number extraction.
    ...
    try:
        ...
        try:
            ...
            preprocessing_methods.append(("denoised", denoised))
            ...
            configs = [
                ...
                (r'--oem 3 --psm 11', "sparse"),  # Sparse text
                (r'--oem 3 --psm 13', "raw"),  # Raw line
            ]
                ...
                for config, config_type in configs:
                    try:
                        ...
                        if text and text.strip():
                            ...
                            if cleaned and len(cleaned) >= 6:
                                confidence = 0.7 if config_type == "strict" else 0.6
                                all_candidates.append((f"tesseract_{method_name}_{config_type}", cleaned, confidence))
                                print(f"[DEBUG] Tesseract {method_name} {config_type} found: {cleaned}")
                    except:
                        continue
                
        except Exception as e:
            print(f"[DEBUG] Tesseract failed for plate: {e}")
        
        # Method 3: Morphological operations + OCR
        try:
            gray = cv2.cvtColor(plate_crop, cv2.COLOR_BGR2GRAY)
            ...
            for config in morph_configs:
                try:
                    ...
                    if text and text.strip():
                        cleaned = _clean_license_plate_text(text)
                        if cleaned and len(cleaned) >= 6:
                            all_candidates.append(("morphological", cleaned, 0.6))
        ...
        if all_candidates:
            ...
            for method, text, conf in all_candidates:
                print(f"[DEBUG]   {method}: {text} (conf: {conf})")
            
            # Filter by valid Indian license plates
            valid_candidates = [(method, text, conf) for method, text, conf in all_candidates 
                              if _is_valid_indian_license_plate(text)]
            
            if valid_candidates:
                # Select the best result among valid candidates
                best_result = _select_best_indian_plate_result([text for _, text, _ in valid_candidates])
                print(f"[DEBUG] Best valid result: {best_result}")
                return best_result
            else:
                # If no valid Indian plates, return the highest confidence candidate
                best_candidate = max(all_candidates, key=lambda x: x[2])
                print(f"[DEBUG] Best candidate (not valid Indian): {best_candidate[1]}")
...
def detect_license_plates_as_objects(image_bgr: np.ndarray) -> list:
    ...
    try:
        ...
        # Method 2: Enhanced detection for challenging images (if no plates found)
        if not license_plate_regions and ENHANCED_DETECTION_AVAILABLE:
            print("[DEBUG] No plates found with YOLO, trying enhanced detection...")
...
with gr.Blocks(title="YOLO26 Object Detection") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Image"):
            with gr.Row():
                with gr.Column():
                    ...
                    img_resnet = gr.Checkbox(value=True, visible=False)  # Always enabled
                    img_max_boxes = gr.Slider(minimum=1, maximum=25, value=10, step=1, label="Max boxes to classify")
                    img_ocr = gr.Checkbox(value=True, visible=False)  # Always enabled
        ...
        with gr.TabItem("Video"):
            gr.Markdown("### Upload a video to process with YOLO26 object detection")
            with gr.Row():
                with gr.Column():
                    ...
                    vid_resnet = gr.Checkbox(value=True, visible=False)  # Always enabled
                    vid_max_boxes = gr.Slider(minimum=1, maximum=25, value=5, step=1, label="Max boxes per frame")
                    vid_every_n = gr.Slider(minimum=1, maximum=30, value=5, step=1, label="Classify every N frames")
                    vid_ocr = gr.Checkbox(value=True, visible=False)  # Always enabled
                    vid_ocr_every_n = gr.Slider(minimum=1, maximum=30, value=5, step=1, label="OCR every N frames")
                ...
                with gr.Column():
                    # Use a more compatible video component setup
                    vid_output = gr.Video(
                    ...
                    # Add download link for backup - always visible
            ...
            def process_video_with_status(video, conf, iou, model, labels, conf_show, imgsz, enable_resnet, max_boxes, every_n, enable_ocr, ocr_every_n):
                ...
                try:
                    # Update status
                    return None, None, "Starting video processing...", "Initializing..."
                    
                except Exception as e:
                    return None, None, f"Error: {str(e)}", f"Processing failed: {str(e)}"

            def process_video_complete(video, conf, iou, model, labels, conf_show, imgsz, enable_resnet, max_boxes, every_n, enable_ocr, ocr_every_n):
                ...
                try:
                    ...
                    # Process video
                    result_path = predict_video(video, conf, iou, model, labels, conf_show, imgsz, enable_resnet, max_boxes, every_n, enable_ocr, ocr_every_n)
            ...
            vid_btn.click(
                ...
                inputs=[vid_input, vid_conf, vid_iou, vid_model, vid_labels, vid_conf_show, vid_size, vid_resnet, vid_max_boxes, vid_every_n, vid_ocr, vid_ocr_every_n],
        ...
        with gr.TabItem("Webcam"):
            with gr.Row():
                with gr.Column():
                    ...
                    webcam_resnet = gr.Checkbox(value=True, visible=False)  # Always enabled
                    webcam_max_boxes = gr.Slider(minimum=1, maximum=25, value=3, step=1, label="Max boxes per frame")
                    webcam_every_n = gr.Slider(minimum=1, maximum=30, value=5, step=1, label="Classify every N frames")
                    webcam_ocr = gr.Checkbox(value=True, visible=False)  # Always enabled
                    webcam_ocr_every_n = gr.Slider(minimum=1, maximum=30, value=5, step=1, label="OCR every N frames")
                with gr.Column():
